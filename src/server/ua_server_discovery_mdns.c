...
mdns_record_remove(UA_Server *server, const char *record,
    ...
    if(dm->serverOnNetworkCallback &&
       !mdns_is_self_announce(server, entry))
        dm->serverOnNetworkCallback(&entry->serverOnNetwork, UA_FALSE,
    ...
    UA_WorkQueue_enqueueDelayed(&dm->workQueue, &entry->delayedCleanup);
#endif
}
    ...
    if(entry->srvSet && server->discoveryManager.serverOnNetworkCallback &&
       !mdns_is_self_announce(server, entry))
        server->discoveryManager.
            serverOnNetworkCallback(&entry->serverOnNetwork, UA_TRUE, entry->txtSet,
